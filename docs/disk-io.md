...
| 操作 | API | 触发路由 | 说明 |
|------|-----|----------|------|
| 目录列举 | `fs.readdirSync` | `GET /fs/list`, `POST /fs/scan`, `POST /fs/backfill`, `POST /fs/sync-file-table` | 同步读取目录条目 |
| 文件/目录 stat | `fs.statSync` | 几乎所有路由 | 获取 mtime、size、isDirectory |
| 文件存在检查 | `fs.accessSync` / `fs.existsSync` | `GET /fs/download`, `GET /fs/file`, `GET /fs/archive/list`, `POST /fs/archive/extract` | 访问权限检查 |
| 文件流读取 | `fs.createReadStream` | `GET /fs/file`, `GET /fs/download`, `GET /fs/archive/file` | 支持 Range 分片 |
| 驱动器列表 | `node-disk-info` (`getDiskInfoSync`) | `GET /fs/drives` | 底层调用 `wmic`(Win) / `df`(Unix) |

## 写操作

| 操作 | API | 触发路由 | 说明 |
|------|-----|----------|------|
| 创建目录 | `fs.mkdirSync` | `POST /fs/ensure-dir`, `POST /fs/move-file`, `POST /fs/move-folder` | `recursive: true` |
| 移动/重命名 | `fs.renameSync` | `POST /fs/move-file`, `POST /fs/move-folder`, `POST /fs/rename` | 同盘原子操作 |
| 删除 | `fs.rmSync` | `DELETE /fs/delete` (permanently=true) | 递归强制删除 |
...
| 清理解压缓存 | `fs.rmSync` | `DELETE /fs/extract-cache` | 删除 `data/extract_cache/` 下内容 |
...
| 操作 | API | 触发路由 | 说明 |
|------|-----|----------|------|
| 目录 watch | `fs.watch` | `POST /fs/scan-watch` | 递归监听，变更时自动 upsert DB |
