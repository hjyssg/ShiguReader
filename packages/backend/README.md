...
##### 开发流程
    etc\sync_frontend_assets_to_backend.py 会先执行 `npm run build`，然后把前端分支生成的 dist 和共享模块（`src/common`）复制到此处作为静态文件去 serve，兼容最新的前端目录结构。名称解析模块现在独立于 `packages/name-parser/`，无需再同步。
