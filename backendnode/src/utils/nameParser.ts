...
function classifyToken(token: string): ClassifyResult {
  if (isMediaType(token)) {
    return { kind: "mediaType", value: getMediaType(token) ?? token };
  }
  if (belongsToEvent(token)) {
    return { kind: "event" };
  }
  if (isStrDate(token)) {
    return { kind: "date" };
  }
  if (isUselessTag(token)) {
    return { kind: "useless" };
  }
  if (isNotAuthor(token)) {
    return { kind: "notAuthor" };
  }
  return { kind: "unknown" };
}
...
export function parseName(filename: string): ParsedName {
  ...
  for (const token of bTokens) {
    ...
    if (cls.kind === "event") {
      if (!result.event) {
        result.event = token; // first event wins
      }
      continue;
    }
    if (cls.kind === "date") {
  ...
  result.rawTags = splitTags.filter(
    (t) => t.length > 1 && !isUselessTag(t) && !isMediaType(t) && !authorSet.has(t) && !t.match(/^\d+$/),
  );
