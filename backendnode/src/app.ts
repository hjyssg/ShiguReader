...
import { cosersRoutes } from "./routes/entities/cosers.js";
...
export function buildApp() {
  ...
  app.get(
    `${config.API_V1_STR}/fs/thumb`,
    { schema: { summary: "获取文件缩略图", tags: ["缩略图"] } },
    async (req, reply) => {
  ...
  // ── Health ─────────────────────────────────────────────────────────────────
  app.get("/health", { schema: { summary: "健康检查", tags: ["系统"] } }, async () => ({
    status: "ok",
    project: config.PROJECT_NAME,
  }));
  // Frontend SDK calls /api/v1/utils/health-check/
  app.get(
    `${config.API_V1_STR}/utils/health-check/`,
    { schema: { summary: "前端 SDK 健康检查", tags: ["系统"] } },
    async () => true,
  );
