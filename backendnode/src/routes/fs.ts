...
export async function fsRoutes(app: FastifyInstance) {
  ...
  app.get("/favorite", { schema: { summary: "获取收藏目录信息", tags: ["文件系统"] } }, getFavorite);
  app.get("/already-read", { schema: { summary: "获取已读目录信息", tags: ["文件系统"] } }, getAlreadyRead);
  app.get("/list", { schema: { summary: "列出目录内容（文件+文件夹）", tags: ["文件系统"] } }, listDirectory);
  ...
  app.post("/backfill", { schema: { summary: "回填元数据和缩略图", tags: ["扫描"] } }, backfill);
  app.post("/scan-watch", { schema: { summary: "启动目录文件监听", tags: ["扫描"] } }, scanWatch);
  ...
  app.post("/ensure-dir", { schema: { summary: "确保目录存在（递归创建）", tags: ["文件操作"] } }, ensureDir);
  ...
  app.delete("/extract-cache", { schema: { summary: "清除解压缓存", tags: ["压缩包"] } }, clearExtractCache);
