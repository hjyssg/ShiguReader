...
export async function scanDirectory(
  ...
  setImmediate(async () => {
    try {
      ...
      try { getRepo().logActivity("scan", `Scan failed: ${dirPath}`, "failed", `scan:${dirPath}`, dirPath); } catch { /* ignore */ }
    }
  });
  ...
  if (!filterPath) {
    for (const wp of watcherPaths) {
      if (!scanStatusMap.has(wp)) {
        entries.push({
          path: wp, status: "completed", message: null, recursive: true,
          scanned_folders: 0, scanned_files: 0, parsed_files: 0,
          watcher_active: true, started_at: null, finished_at: null,
        });
      }
    }
  }
    ...
    const watcher = fs.watch(dirPath, { recursive: true }, (_event, filename) => {
      ...
      setImmediate(() => {
        try {
          ...
          getRepo().upsertFile({
            filepath: fullPath, folderpath: dirPath, filename: path.basename(fullPath),
            mtime: Math.floor(s.mtimeMs / 1000), filesize: s.size,
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        repo.upsertFile({
          filepath: fullPath, folderpath: dirPath, filename: entry.name,
          mtime: Math.floor(s.mtimeMs / 1000), filesize: s.size,
          file_type: fileType, ext: path.extname(entry.name).toLowerCase() || null,
        });
        scannedFiles++;
    ...
    try { getRepo().logActivity("backfill", `Backfill failed: ${dirPath}`, "failed", `backfill:${dirPath}`, dirPath); } catch { /* ignore */ }
  }
